...
- `npx vitest run packages/connectors/src/adapters/html-ics.test.ts`
- `npm test`

## Completion Notes List

- Introduced a routed worker executor that delegates by job payload, restoring existing connector validation flows while wiring a new connector validation executor for OAuth providers.
- Updated the HTML/ICS adapter to reuse cached previews on HTTP 304 responses via persisted validation metadata and cache headers.
- Added targeted unit coverage for the worker router, connector validation executor, and conditional-fetch handling; full `npm test` passes with worker integration suites skipped when PostgreSQL is unavailable.
...
- apps/worker/src/executors/connector-validation.ts
- apps/worker/src/executors/connector-validation.test.ts
- apps/worker/src/executors/sync.ts
- apps/worker/src/executors/sync.test.ts
- apps/worker/src/executors/utils.ts
