...
- [@syncal/connectors/src/adapters/html-ics.ts](packages/connectors/src/adapters/html-ics.ts)
- [apps/api/src/services/connector-validation.ts](apps/api/src/services/connector-validation.ts)
- [apps/web/src/app/(connectors)/connectors/page.tsx](apps/web/src/app/(connectors)/connectors/page.tsx)
- [apps/web/src/app/(connectors)/connectors/submission-feedback.ts](apps/web/src/app/(connectors)/connectors/submission-feedback.ts)

## Completion Notes List

- Created shared HTML/ICS adapter with timeout-aware fetch, ICS parsing, timezone + recurrence handling, and validation result normalization reused across API and future workers.
- Wired API services to persist masked feed metadata, encrypted credentials, and new `last_successful_fetch_at` column, aligning DTOs and response payloads with architecture guidance.
- Implemented portal wizard flow for HTML/ICS feeds including inline field validation, feed testing via `POST /connectors/validate`, preview rendering, and enhanced connector list display.
- Added deterministic worker executor for HTML/ICS sync jobs with caching/resume logic plus Vitest coverage for adapter edge cases and submission feedback messaging; verified `npm test` covers worker adapters and services.
- Extended connector list UI with validation issue callouts, retry affordances, and Playwright coverage simulating failed/successful feed validation while documenting schema updates for fetch caching.
- Confirmed `npm test` (with `TEST_DATABASE_URL=postgresql://syncal:syncalsecret@127.0.0.1:5432/syncal`) passes all 80 tests, including worker integration suites after migrations create `citext`/`uuid-ossp`.
...
- packages/connectors/src/utils/mask.ts
- packages/connectors/src/index.ts
- apps/api/src/services/connector-validation.ts
- apps/api/src/services/connectors.ts
- apps/api/src/plugins/connectors.ts
- apps/api/src/lib/mask.ts
- apps/api/src/repositories/connector.repository.ts
- apps/api/tests/integration/connectors.oauth.test.ts
- apps/api/tests/unit/connectors.html-ics.test.ts
- apps/api/tests/integration/connectors.html-ics.validation.test.ts
...
- prisma/migrations/20250923112216_add_connector_last_successful_fetch_at/migration.sql
- packages/core/src/dtos/connectors.ts
- apps/worker/src/main.ts
- apps/worker/src/executors/html-ics.ts
- apps/worker/tests/integration/html-ics.sync.test.ts
