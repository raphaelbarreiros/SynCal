...
- [ ] Implement HTML/ICS connector wizard support (AC: 1, 2, 3)
  ...
  - [ ] Trigger the API validation endpoint from the wizard “Test feed” action and render the five-event preview plus actionable error messaging via the shared submission feedback component. [Source: docs/architecture/core-workflows.md#connector-onboarding--validation; docs/architecture/front-end-spec.md#connector-setup-oauth--html-ics--imap--self-managed]

- [ ] Add HTML/ICS connector adapter and validation flow (AC: 2, 3)
  - [ ] Create `packages/connectors/src/adapters/html-ics` implementing `ConnectorAdapter.validate` to fetch the feed with optional auth header/token, enforce 15s HTTP timeout, and parse events using node-ical. [Source: docs/architecture/components.md#connectors-library; docs/architecture/external-apis.md#htmlics-feeds; docs/architecture/error-handling-strategy.md#external-api-errors]
...
- [ ] Extend API connectors service for HTML/ICS configuration (AC: 1, 2, 3)
  - [ ] Update DTOs and Zod schemas for `POST /connectors` to accept the `html_ics` type with feed URL, masked display label, and optional auth header/token stored encrypted via libsodium helpers. [Source: docs/architecture/rest-api-spec.md#/connectors; docs/architecture/database-schema.md; docs/architecture/security.md#secrets-management]
  ...
  - [ ] Add a migration to persist `last_successful_fetch_at` for HTML/ICS connectors (either new column or config payload) and ensure the API response returns the masked URL, preview events, and timestamp currently used by the portal list. [Source: docs/architecture/database-schema.md; docs/architecture/core-workflows.md#connector-onboarding--validation]
  - [ ] Persist connector `config_json` with masked URL plus preview metadata, update `last_validated_at` on success, and return validation results—including preview events and validation status—to the portal. [Source: docs/architecture/database-schema.md; docs/architecture/core-workflows.md#connector-onboarding--validation]
...
- [ ] Schedule and process HTML/ICS sync jobs (AC: 2, 4)
  ...
  - [ ] Store and reuse `ETag`/`Last-Modified` headers from successful fetches so subsequent syncs can perform conditional requests and reduce load, falling back gracefully when headers are absent. [Source: docs/architecture/external-apis.md#htmlics-feeds; docs/architecture/core-workflows.md#sync-job-lifecycle]
...
- Data Models
  - `connectors` rows store type (`html_ics`), status, encrypted credentials (including raw feed URL + optional auth header/token), JSON config, validation timestamps, and the new `last_successful_fetch_at` metric surfaced in the portal list; related calendar rows capture provider IDs, display names, privacy modes, and metadata. [Source: docs/architecture/database-schema.md]
  - Document the schema change (`last_successful_fetch_at`) in `docs/architecture/database-schema.md` alongside any new encrypted fields so architecture references stay current. [Source: docs/architecture/database-schema.md]

- API Specifications
  - `POST /connectors` creates connectors and triggers validation, returning masked URL (format: `<scheme>://<host>/…/<last-path>`), preview events, and `last_successful_fetch_at`; `GET /connectors` and `PATCH /connectors/{id}` surface the same metadata used by the portal; scheduling validations leverages `POST /jobs/schedule`. [Source: docs/architecture/rest-api-spec.md#/connectors; docs/architecture/core-workflows.md#connector-onboarding--validation]
...
- External Provider Contracts
  - HTML/ICS feeds are accessed via user-supplied HTTPS URLs with optional auth headers; use node-ical, respect `ETag`/`Last-Modified` (persist values for conditional requests), validate HTTP 200 and `text/calendar`, and treat duplicates or partial data gracefully. [Source: docs/architecture/external-apis.md#htmlics-feeds]

- Error Handling & Retries
  - Apply the shared external API error strategy: HTTP timeout defaults (15s), exponential backoff with jitter, and connector failure counters that pause connectors after repeated failures. [Source: docs/architecture/error-handling-strategy.md#external-api-errors]
