...
- [x] Add `.env.example` with `DATABASE_URL`, `SESSION_SECRET` (for cookie sessions), `ENCRYPTION_KEY`, `INITIAL_ADMIN_EMAIL`, `INITIAL_ADMIN_PASSWORD` [Source: docs/architecture/tech-stack.md; docs/architecture/security.md]
...
- Synced local development guide instructions with `ENCRYPTION_KEY`/`INITIAL_ADMIN_EMAIL` expectations and re-ran `npm test` post-update.
...
# QA Results
- ✅ `docs/architecture/local-development.md:19-71` now references `ENCRYPTION_KEY`, documents `INITIAL_ADMIN_EMAIL`, and limits first-run checks to `/healthz`, so following the guide lets the compose stack boot successfully.
- ✅ `npm test`
- ✅ `npm run smoke`
- ℹ️ Broader docs (e.g., ops runbook) still mention `ENCRYPTION_MASTER_KEY`; plan a follow-up sweep so env naming stays consistent.
